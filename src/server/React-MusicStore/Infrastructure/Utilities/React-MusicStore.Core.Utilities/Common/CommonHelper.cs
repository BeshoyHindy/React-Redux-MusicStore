...
namespace ReactMusicStore.Core.Utilities.Common
{
	public static partial class CommonHelper
	{
		...
		public static string MapPath(string path, bool findAppRoot = true)
		{
			...
			else
			{
				...
				if (findAppRoot /* && !Directory.Exists(testPath)*/)
				{
					...
					if (dir != null)
					{
						baseDirectory = Path.Combine(dir.FullName, "Presentation\\React-MusicStore.WebApi");
		...
		public static string MapPath_test(string path, bool findAppRoot = true)
		{
			//Guard.NotNull(path, nameof(path));
			...
			//        if (dir != null)
			//        {
			//            baseDirectory = Path.Combine(dir.FullName, "DemoDataSettings.WebAPI");
			//            testPath = Path.Combine(baseDirectory, path);
			//        }
			//    }
		...
		{
			return File.Exists(Path.Combine(dir.FullName, "React-MusicStore.sln"));
		}
