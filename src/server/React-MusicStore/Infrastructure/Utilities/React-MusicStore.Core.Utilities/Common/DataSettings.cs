...
namespace ReactMusicStore.Core.Utilities.Common
{
	public partial class DataSettings
	{
		...
		//protected const string FILENAME = "Settings.local.txt";

		//----- for update online DB -------
		// protected const string FILENAME = "Settings.online.txt";
				...
				if (File.Exists(filePath))
				{
					...
					if (settings.Any())
					{
						...
						if (settings.ContainsKey("DataConnectionString"))
						{
							...
							//this.DataConnectionString = @"Data Source=.\SQLEXPRESS;Initial Catalog=PomegranateDB_Demo;Integrated Security=True;Persist Security Info=False;Enlist=False;Pooling=True;Min Pool Size=1;Max Pool Size=100;Connect Timeout=15;User Instance=False";

							//return @"Data Source=.\SQLEXPRESS;Initial Catalog=PomegranateDB_Demo;Integrated Security=True;Persist Security Info=False;Enlist=False;Pooling=True;Min Pool Size=1;Max Pool Size=100;Connect Timeout=15;User Instance=False";
		...
		{
			string filePath = Path.Combine(CommonHelper.MapPath_test("~/App_Data/"), FILENAME);

			//var filePath = @"C:\MyProjects\DemoDataSettings\DemoDataSettings\DemoDataSettings.WebAPI\App_Data/Settings.txt";

			if (File.Exists(filePath))
			{
				string text = File.ReadAllText(filePath);
				var settings = ParseSettingsStatic(text);
				if (settings.Any())
				{
					//this.RawDataSettings.AddRange(settings);
					//if (settings.ContainsKey("AppVersion"))
					...
					return settings["DataConnectionString"];
					// return @"Data Source=.\SQLEXPRESS;Initial Catalog=PomegranateDB_Demo;Integrated Security=True;Persist Security Info=False;Enlist=False;Pooling=True;Min Pool Size=1;Max Pool Size=100;Connect Timeout=15;User Instance=False";
		...
		protected static IDictionary<string, string> ParseSettingsStatic(string text)
		{
			var result = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
